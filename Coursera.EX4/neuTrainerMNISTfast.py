...
def BackProp(thetaAll, xArr, yArr):
	...
	a2 = hypothesis(theta1, xArr)					# 36 x g.n
	a2 = np.vstack(( np.asarray([1 for i in range(g.n)]) , a2))	# Reshape to 37 x g.n (Add bias row)
	a3 = FullHypo(theta1, theta2, xArr)				# 10 x g.n
	
	# Creating (Capital) Delta matrices
	Delta2 = np.zeros((10, g.f2+1))			# 10 x 37
	Delta1 = np.zeros((g.f2, g.f1+1))		# 36 x 785
	
	# Calculate (Lowercase) deltas for each element in the dataset and add it's contributions to the Deltas
	delta3 = a3.T - yArr						# g.n x 10
	gPrime = np.multiply(a2.T, 1 - a2.T)				# g.n x 37
	delta2 = np.multiply(np.matmul(delta3, theta2), gPrime)		# g.n x 37
	Delta2 = Delta2 + np.matmul(a2, delta3).T

	# Note: we use the bias row to calculate Delta2 but not Delta1, so it is removed below
	Delta1 = Delta1 + np.delete( np.matmul(xArr.T, delta2).T , 0, 0)	
...
print 'Initial Theta JCost: ', RegJCost(thetaAll, xArr, yArr)  # Outputting around 12.332

# Check the gradient function. ~1.43362624525e-05 for randomized thetas
