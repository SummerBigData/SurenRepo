...
def GenDat():
	...
	totStart = time.time()
	print 'Running dataPrep.py, GenDat'
	...
	totend = time.time()
	print"dataPrep.py's GenDat took ", totend - totStart, 'seconds to run'
	print ' '
	return data
...
def GenSubTrain():
	# To see how long the code runs for, we start a timestamp
	totStart = time.time()
	print 'Running dataPrep.py, GenSubTrain'

	# Get the data. It is 64 x 64 x 3 x 2000
	data = scipy.io.loadmat('data/stlTrainSubset.mat')['trainImages']

	ordDat = np.zeros((2000, 64, 64, 3))
	for i in range(2000):
		ordDat[i] = data[ :, :, :, i]

	# Stop the timestamp and print out the total time
	totend = time.time()
	print"dataPrep.py's GenSubTrain took ", totend - totStart, 'seconds to run'
	print ' '
	return ordDat
...
def GenSubTest():
	# To see how long the code runs for, we start a timestamp
	totStart = time.time()
	print 'Running dataPrep.py, GenSubTest'

	# Get the data. It is 64 x 64 x 3 x 3200
	data = scipy.io.loadmat('data/stlTestSubset.mat')['testImages']
	
	ordDat = np.zeros((3200, 64, 64, 3))
	for i in range(3200):
		ordDat[i] = data[ :, :, :, i]

	# Stop the timestamp and print out the total time
	totend = time.time()
	print"dataPrep.py's GenSubTest took ", totend - totStart, 'seconds to run'
	print ' '
	return ordDat
...
#	X = X.reshape((datl, pixl*coll))
	...
	S = S.reshape(len(S), 1)
	l_mat = np.diag(1.0/np.sqrt(S.flatten() + eps))
	ZCA_mat = U.dot(l_mat).dot(U.T)
	return X.dot(ZCA_mat), ZCA_mat, X, mu
