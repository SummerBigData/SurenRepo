...
def hypothesis(theta, x):
	...
	xArr = np.array([[1]*len(x), x])
	thetaArr = np.asarray(theta)
	hypo = [0 for j in range(len(x))]
...
for g in range(200):
	...
	temptheta[0] = theta[0] - 0.07 * (1.0 / len(x)) * np.sum( hypothesis(theta, x) - y )

    # Fill the temporary theta list for the slope
	temptheta[1] = theta[1] - 0.07 * (1.0 / len(x)) * np.sum( np.matmul(hypothesis(theta, x)- y, x) )  
